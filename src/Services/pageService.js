...
export function getPages(callback, applicationId) {
    ...
    axios.get(`${qfservice}/webpages/getWebPagesList?module_id=${applicationId}`).then(res => {
...
export function getElementsDetails(callback, element_id) {
    axios.get(`${qfservice}/webpages/getWebPageElementPathsInfo?element_id=${element_id}&is_diff_page_element=false`).then(res => {
        callback(res.data.info)
    })
}
...
export function getScreen(callback, appId) {
    ...
    axios.get(`${qfservice}/screen/getScreensList?module_id=${appId}`).then(res => {
        if (res.data.info != null) {
            callback(res.data.info)
        }
    })
