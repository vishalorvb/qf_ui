...
export async function getAlltestcaseOfApplicationandSubapplication(
  ...
  return await axios
    .get(
      ...
      `${qfservice}/qfservice/getModuleAndTestcases?projectId=${projectId}&moduleId=${moduleId}&page=${page}&size=${size}`
    )
    .then((res) => {
...
export async function getCustomCodeList(moduleId, userId, callback) {
  return await axios
    .get(
      `${qfservice}/qfservice/customcode/289?module_id=${768}&user_id=${userId}`
    )
    .then((res) => {
      callback(res.data);
    });
