...
export default function TestCases(props) {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (param) => {
        return (
          <div>
            {param.row.is_db_dataset === true
              ? "Database"
              : param.row.is_regression === true
              ? "Regression"
              : "Regular"}
          </div>
        );
  ...
  const getModuleTestCases = () => {
    axios
      .get(
        baseUrl +
          `/ProjectsMS/Project/getModuleTestCases?moduleId=${module?.module_id}`
      )
      .then((resp) => {
  ...
  const getDatasets = () => {
    axios
      .get(
        baseUrl +
          `/ProjectsMS/Project/getModuleTestCaseDatasets?testcase_id=${selectedTestcase?.testcase_id}`
      )
      .then((resp) => {
  ...
  return (
    <Box>
      ...
      >
        <Pillnav
          workflowModules={testCaseList}
          selectClickedElement={console.log}
        />
        ...
        <Grid
          ...
          gap={1}
        >
          <Button variant="contained">Create TestCase</Button>
          <Button
            variant="contained"
            id="basic-button"
