...
export function validateForm(
  ...
  specialcharRefs.forEach((element) => {
    try {
      if (
        ...
        element.current.value.trim() === "" ||
        element.current.value.trim() === undefined
      ) {
        element.current.className = cname;
        status = false;
  ...
  passwordRef.forEach((element) => {
    try {
      if (
        !containUpperCase(element.current.value) ||
        !containsSpecialChars(element.current.value) ||
  ...
  onlyalphabets.forEach((element) => {
    try {
      if (
        !onlyAlphabets(element.current.value) ||
        element.current.value.trim() === "" ||
        element.current.value.trim() === undefined
      ) {
  ...
  onlynumbers.forEach((element) => {
    try {
      if (
        element.current.value.trim() === "" ||
        onlyNumber(element.current.value) === false
      ) {
        element.current.className = cname;
        status = false;
