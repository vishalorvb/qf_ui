...
function ApiAuthorization({ ApiDetails }) {
  let Auth = useRef(
    JSON.parse(
      ApiDetails.auth == null
        ? '{"authtype":"basicauth","basicauth":{"username":"","password":""},"apikey":{"key":"","value":"","addto":"header"},"bearertoken":{"token":"token"},"oauth2":{"tokenurl":"","clientid":"","clientsecret":""}}'
        : ApiDetails.auth.auth_data
    )
  );
      ...
      >
        <Grid item md={3}>
          ...
          >
            ...
            <option
              selected={ApiDetails?.auth?.authtype === "noauth"}
              value={"noauth"}
            >
              No Auth
            </option>
            <option
              selected={ApiDetails?.auth?.authtype === "basicauth"}
              value={"basicauth"}
            >
              Basic Auth
            </option>
            <option
              selected={ApiDetails?.auth?.authtype === "apikey"}
              value={"apikey"}
            >
              API Key
            </option>
            <option
              selected={ApiDetails?.auth?.authtype === "bearertoken"}
              value={"bearertoken"}
            >
              Bearer Token
            </option>
            <option
              selected={ApiDetails?.auth?.authtype === "oauth2"}
              value={"oauth2"}
            >
              OAuth 2.0
            </option>
