...
function DeletePhase(props) {
    const { openDelete, setOpenDelete,object,setDelSuccessMsg, getPhases, phases, setPhases } = props;
    console.log(object)
    ...
    const submit = () => {
        phaseId && axiosPrivate.post(`/Biservice/projects/phases/delete?phase_id=${phaseId}`).then(
            res => {
        ...
        <Dialog open={openDelete} onClose={handleClose} style={{ marginLeft: "15px", marginTop: "20px" }}>
                <DialogTitle id="alert-dialog-title" className="dialogTitle border-bottom" sx={{
                    ...
                    <Grid container direction="row" justify="space-between" alignItems="center" className="poptitle">
                        <Typography sx={{ marginLeft: 1 }} variant="inherit">Delete Phase </Typography>
