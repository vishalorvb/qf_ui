...
export default function CreateAnsibleInstance() {
    ...
    const onSubmitHandler = (data) => {
        ...
        axios
            .post(
                `${qfservice}/qfservice/CreateWebRelease?release_id=${location?.state?.id ? location?.state?.id : 0
                }&module_id=${location?.state?.module_id}&release_name=${data?.releaseName
                }&release_desc=${data?.releaseDesc}&app_server_type=${data?.serverType
                }&app_server_url=${data?.serverUrl}&app_server_credentials_id=${data?.serverCredentialId
                }&war_name=${data?.warName}&app_source_code_branch_name=${data?.repoBranch
                }&app_source_code_repo_access_token=${data?.repoAccessToken}&username=${data?.repoUsername
                }&git_url=${data?.repoUrl
